...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || $content === '' || !array_key_exists($category, $categories)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($error === '') {
                logAuditAction('add_news', 'post', $post_id, null, ['title' => $title, 'category' => $category]);
...
<div class="admin-card amsa-card p-4">
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="admin-form-actions">
            <button type="submit" class="btn btn-primary amsa-btn amsa-btn-primary">Publish</button>
